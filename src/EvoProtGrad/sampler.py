...
def prettify_tensor(x, res=4):
    return x.squeeze().detach().cpu().numpy().round(res)
...
class DirectedEvolution:
    ...
    def __init__(self, 
        ...
        # Modify tokenizer initialization in the sampler class
        #self.canonical_chain_tokenizer = tokenizers.OneHotTokenizer(
        #    alphabet=self.experts[0].model.alphabet.all_toks  # Use the full ESM alphabet
        #)
        self.canonical_chain_tokenizer = tokenizers.OneHotTokenizer(
    ...
    def _product_of_experts(self, inputs: List[str]) -> Tuple[List[torch.Tensor], torch.Tensor]:
        ...
        for expert in self.experts:
            ...
            #print(f"Evolution scored sequences {convert_full_to_relative_sequences(inputs2, self.wtseq.replace(" ", ""))} to get {scores2}")
    ...
    def _compute_gradients(self, ohs: List[torch.Tensor], PoE: torch.Tensor) -> torch.Tensor:
        ...
        #print(f"oh_grads have shape {oh_grads[0].shape}")
        ...
        # sum over experts
        #print(f"summed_grads have shape {summed_grads[0].shape}")
    ...
    def __call__(self) -> Tuple[List[str], np.ndarray]:
        ...
        for i in range(self.n_steps):
            ...
            if i > 10 and i % 100 == 0:
                print(f"started step {i} out of {self.n_steps}.")
            dec_seqs  =  self.canonical_chain_tokenizer.decode(cur_chains_oh)
            # print(f"dec seqs starts as: {prep_seqs(dec_seqs, self.wtseq)}")
            #print(f"Cur chains: {prep_seqs(self.chains, self.wtseq)}. Decoded chains: {prep_seqs(dec_seqs, self.wtseq)}.")
            start_seqs = self.chains.copy()
            # Check for mismatches between self.chains and decoded sequences
            # for idx, (chain, decoded_chain) in enumerate(zip(prep_seqs(self.chains, self.wtseq), prep_seqs(dec_seqs, self.wtseq))):
            #     if chain != decoded_chain:
            #         print(f"Mismatch in chain {idx} at beginning of iteration {i}:")
            #         print(f"Original chain: {chain}")
            #         print(f"Decoded chain:  {decoded_chain}")
            self.chains = dec_seqs # to reflect any reset chains that reached multiple mutations
            ###### sample path length
            ...
            #seqs = prep_seqs(y_strs, self.wtseq)
            ...
            with torch.no_grad():
                ...
                # print(f"Shapes: y {y.shape}, accepted {accepted.shape}, cur_chains_oh {cur_chains_oh.shape}.")
                ...
                # print(f"proposed seqs: {prep_seqs(dec_seqs, self.wtseq)}")
                ...
                #            print(f"##### Chain {chain_idx} changed but should not have. Accepted: {accepted.squeeze()[chain_idx]}")
                #cur_chains_oh = cur_chains_oh2
                # print(f"Proposed PoE: {prettify_tensor(proposed_PoE)}. Cur PoE: {prettify_tensor(PoE)}.")
                # print(f"Shapes: proposed_PoE {proposed_PoE.shape}, accepted {accepted.shape}, PoE {PoE.shape}.")
                # print(f"Shapes after squeeze: proposed_PoE {proposed_PoE.squeeze().shape}, accepted {accepted.squeeze().shape}, PoE {PoE.squeeze().shape}.")
                #PoE = proposed_PoE.squeeze() * accepted.squeeze() + PoE.squeeze() * (1. - accepted.squeeze())        
                # print(f"Accepted: {prettify_tensor(accepted)}. Final PoE: {prettify_tensor(PoE)}.")

            # Current chain state book-keeping    
            self.chains_oh = cur_chains_oh.clone()
            #print(f"cur_chains_oh has shape {cur_chains_oh.shape}")
            ...
            # print(f"final seqs: {prep_seqs(dec_seqs, self.wtseq)}")
            # Update only for accepted chains
            #for idx in range(self.parallel_chains):
            #    if accepted.squeeze()[idx] == 1:  # Only update if accepted
            #        self.chains[idx] = y_strs[idx]  # Directly assign the decoded sequence from y_strs
            #self.chains = decoded_sequences.copy()
            # Check for mismatches after updating chains
            # for idx, (chain, decoded_chain) in enumerate(zip(prep_seqs(start_seqs, self.wtseq), prep_seqs(decoded_sequences, self.wtseq))):
            #     if chain != decoded_chain:
            #         print(f"Sequence changed in chain {idx} at iteration {i}:")
            #         print(f"Original seq: {chain}")
            #         print(f"New seq:  {decoded_chain}. Accepted was {accepted.squeeze()[idx]}")
            ...
            if self.max_mutations > 0:
                ...
                # Also reset the corresponding sequences in self.chains to the WT sequence
                #for idx, flag in enumerate(mask_flag):
                #    if flag:
                #        self.chains[idx] = self.wtseq
        ...
        if self.output == 'last':
            ...
            for j in range(len(self.chains_oh_history)):
                ...
                # print(f"Final sequences at step {j}: {seqs}")
                # print(f"Corresponding scores at step {j}: {scores}")
    
                for k, seq in enumerate(decoded_sequences):
                    score2 = self._recompute_score(seq)
                    if not np.isclose(scores[k], score2, atol=1e-1):
                        seq2 = convert_full_to_relative_sequences([seq.replace(" ", "")], self.wtseq.replace(" ", ""))
                        # print(f"Mismatch found at step {j}, chain {k}: Sequence {seq2} has mismatched score {(scores[k], float(score2))}")
                        
                output_ += [ decoded_sequences ]
                #print(f"Final history chain at step {i}: {self.chains_oh_history[i]}")
                # print(f"Final history score at step {j}: {self.PoE_history[j]}")
